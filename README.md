...
But are Vine reviews truly trustworthy? The task is to investigate whether Vine reviews are free of bias. Use either PySpark or—for an extra challenge—SQL to analyze the data.

* Used Spark on Colab to extract and transform the data and load it into a SQL table on your RDS account. Performed the analysis with SQL queries on RDS.

* Considered steps that can be taken to reduce noisy data, e.g., filtering for reviews that meet a certain number of helpful votes, total votes, or both.

* Submited a summary of the findings and analysis.
